...
import hProjekt.controller.gui.mapEditor.MapSelectionController;
import hProjekt.controller.gui.menus.AboutSceneController;
import hProjekt.controller.gui.menus.EndScreenSceneController;
import hProjekt.controller.gui.menus.LeaderboardSceneController;
import hProjekt.controller.gui.menus.MainMenuSceneController;
import hProjekt.controller.gui.menus.SetupGameSceneController;
...
public class SceneSwitcher {
    ...
    public static SceneSwitcher getInstance(final Stage stage, final Consumer<GameController> gameLoopStarter) {
        if (SceneSwitcher.INSTANCE == null) {
            SceneSwitcher.INSTANCE = new SceneSwitcher(stage, gameLoopStarter);
        }
    ...
    public enum SceneType {
        GAME_BOARD(() -> {
            ...
            return new GameBoardController(SceneSwitcher.getInstance().gameController.getState(),
                    SceneSwitcher.getInstance().gameController.activePlayerControllerProperty(),
                    SceneSwitcher.getInstance().gameController.roundCounterProperty());
        }),

        MAIN_MENU(MainMenuSceneController::new),
        ABOUT(AboutSceneController::new),
        SETUP_GAME_MENU(() -> {
            if (SceneSwitcher.getInstance().gameController != null) {
                SceneSwitcher.getInstance().gameController.stop();
            }
            GameState gameState = new GameState(new HexGridImpl(), new ArrayList<>());
            SceneSwitcher.getInstance().gameController = new GameController(gameState);
            return new SetupGameSceneController(gameState);
        }),
        LEADERBOARD(LeaderboardSceneController::new),
        END_SCREEN(() -> {
            List<Player> players = SceneSwitcher.getInstance().gameController.getState().getPlayers();
            return new EndScreenSceneController(players, SceneSwitcher.getInstance().gameController);
        }),
        MAP_EDITOR(MapSelectionController::new);
    ...
    public void loadScene(final SceneType sceneType) {
        Platform.runLater(() -> {
            ...
            loadScene(newController);
        });
    }

    @DoNotTouch
    public void loadScene(final SceneController controller) {
        Platform.runLater(() -> {
            final Region newRoot = controller.buildView();
            if (stage.getScene() == null) {
                final Scene initialScene = new Scene(new StackPane(), 800, 600); // Default
