 ...
 */
@DoNotTouch
public record TilePosition(int q, int r) implements Comparable<TilePosition>, Serializable {
    ...
    public static Set<TilePosition> neighbours(final TilePosition position) {
        return Arrays.stream(EdgeDirection.values()).map(direction -> TilePosition.neighbour(position, direction))
                .collect(Collectors.toSet());
    }

    /**
     * Executes the given {@link Predicate} on each tile position on a ring
     * with the given radius around the given center.
     * If the given function returns {@code true}, the functions exits early.
     *
     * @param center   the center tile position of the ring
     * @param radius   the radius of the ring
     * @param function the {@link Predicate} to execute, gets the current position
     */
    public static boolean forEachRing(
            final TilePosition center, final int radius,
            final Predicate<TilePosition> function) {
        if (radius == 0) {
            return function.test(center);
        }
    ...
    /**
     * Executes the given {@link Predicate} on each tile position on a spiral with
     * the given radius around the given center.
     * If the given predicate returns {@code true}, the functions exits early.
     *
     * @param center     the center tile position of the spiral
     * @param radius     the radius of the spiral including the center
     * @param function   the {@link Predicate} to execute on each position
     * @param skipCenter whether to skip the center position
     */
    public static void forEachSpiral(
            final TilePosition center, final int radius,
            final Predicate<TilePosition> function,
            final boolean skipCenter) {
        for (int i = skipCenter ? 1 : 0; i <= radius; i++) {
            if (TilePosition.forEachRing(center, i, function)) {
                return;
            }
        }
    }
    ...
    public enum EdgeDirection {
        ...
        public static final List<EdgeDirection> VALUES = List.of(EdgeDirection.values());
        public static final int SIZE = EdgeDirection.VALUES.size();
    ...
    public boolean equals(final Object o) {
        if (!(o instanceof final TilePosition that)) {
            return false;
        }
