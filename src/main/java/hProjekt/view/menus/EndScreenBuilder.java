...
import hProjekt.view.utils.Confetti;
...
public class EndScreenBuilder implements Builder<Region> {
     ...
     */
    public EndScreenBuilder(final Runnable loadMainMenuAction, final List<Player> players) {
    ...
    public Region build() {
        ...
        sortedPlayers.sort(Comparator.comparingInt(Player::getTotalGoldCardValue).reversed());

        final int currentHighscore = LeaderboardController.loadLeaderboardData().stream()
                .min((score1, score2) -> Integer.compare(score2.getScore(), score1.getScore()))
                .map(LeaderboardEntry::getScore)
                .orElse(0);

        // Root container for the entire screen
        final StackPane rootContainer = new StackPane();
        rootContainer.setStyle("-fx-background-color: #1f1f2e;");

        // Add confetti background
        final Pane confettiPane = createConfettiBackground();
        confettiPane.setPickOnBounds(false);
        rootContainer.getChildren().add(confettiPane);

        // Leaderboard container
        final VBox leaderboard = new VBox(20);
        ...
        // Leaderboard title
        final Label title = new Label("Congratulations, " + sortedPlayers.getFirst().getName() + "!");
        title.setStyle(
        ...
        // Highscore Label
        final HBox highscoreBox = new HBox();
        ...
        final Label highscoreLabel = new Label("Highscore: " + currentHighscore);
        ...
        // Table settings
        final double rankWidth = 70;
        final double namePlaceholderWidth = 150;
        final double scorePlaceholderWidth = 60;
        ...
        for (int i = 0; i < sortedPlayers.size(); i++) {
            ...
            // Rank column
            final HBox rankBox = new HBox();
            rankBox.setAlignment(Pos.CENTER);
            rankBox.setMinWidth(rankWidth);
            if (i < 3) {
                final ImageView medal = new ImageView();
            ...
            // Player name
            final Label nameLabel = new Label(player.getName());
            ...
            // Star rating
            final int credits = player.getTotalGoldCardValue();
            final int stars = Math.max(1, Math.min(credits / CREDITS_PER_STAR, 5));
            final HBox starsBox = new HBox(5);
            ...
            // Credits
            final Label creditsLabel = new Label(String.valueOf(credits));
        ...
        final Button backToMenuButton = new Button("Back to Main Menu");
    ...
    private Pane createConfettiBackground() {
        final Pane confettiPane = new Pane();
        confettiPane.setPickOnBounds(false);
        for (int i = 0; i < 1000; i++) {
            final Confetti confetti = new Confetti(randomColor(), 800, 600);
            confetti.animate();
